...
function itemsForSale() {
    connection.query("SELECT item_id, product_name, price, department_name, stock_quantity FROM products WHERE price > 0;", function (err, result) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            table.push([result[i].item_id, wrap(result[i].product_name), result[i].price.toFixed(2), result[i].department_name, result[i].stock_quantity]);
        }
...
function checkStock(stock_quantity, buy_quantity, price, item_id) {
    if (stock_quantity >= buy_quantity) {
